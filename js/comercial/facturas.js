...
async function verDetalleFactura(id) {
  ...
  if (trackBadgeEl && typeof getEstadoTracking === 'function') {
    getEstadoTracking('factura', String(f.id)).then(estado => {
      trackBadgeEl.innerHTML = estado.compartido
        ? `<span onclick="mostrarDetalleTracking('factura','${f.id}','${(f.numero||'').replace(/'/g,'')}')" style="cursor:pointer">${badgeTracking(estado, f.id)}</span>`
        : '';
    });
  }
