...
async function abrirFicha(id, tabActiva) {
  ...
  if (sub) sub.textContent = [c.tipo||'Particular', c.direccion_fiscal, c.municipio_fiscal, c.provincia_fiscal].filter(Boolean).join(' · ');
    ...
    <div>
      ...
      ${(docs.data||[]).length ? '<div style="font-size:10px;color:var(--gris-400);font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:6px 0 2px">Documentos del cliente</div>' + (docs.data||[]).map(d => `
        <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--gris-100);cursor:pointer" onclick="verPdfFirmado('${d.url}','${(d.nombre||'Documento').replace(/'/g,'')}')">
          ...
          <div style="flex:1;min-width:0"><div style="font-weight:700;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${d.nombre}</div><div style="font-size:10.5px;color:var(--gris-400)">${d.tipo} · ${new Date(d.created_at).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})}</div></div>
      ...
      ${docsObras.length ? '<div style="font-size:10px;color:var(--azul);font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:8px 0 2px;margin-top:4px;border-top:1px solid var(--gris-100)">Documentos de obras</div>' + docsObras.map(d => `
        <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--gris-100);cursor:pointer" onclick="verPdfFirmado('${d.url}','${(d.nombre||'Documento').replace(/'/g,'')}')">
          <span style="font-size:18px">${TIPO_ICO[d.tipo]||'📄'}</span>
          <div style="flex:1;min-width:0"><div style="font-weight:700;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${d.nombre}</div><div style="font-size:10.5px;color:var(--gris-400)">${d.tipo} · ${new Date(d.created_at).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})} · <span style="color:var(--azul)">🏗️ ${d._obraNumero}${d._obraTitulo ? ' — '+d._obraTitulo : ''}</span></div></div>
        </div>`).join('') : ''}
      ${docsGenerados.length ? '<div style="font-size:10px;color:#059669;font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:8px 0 2px;margin-top:4px;border-top:1px solid var(--gris-100)">Documentos firmados</div>' + docsGenerados.map(d => `
        <div style="display:flex;align-items:center;gap:8px;padding:7px 0;border-bottom:1px solid var(--gris-100);cursor:pointer" onclick="verPdfFirmado('${d.archivo_url}','${(d.numero||d.tipo_documento)} — PDF Firmado')">
          ...
          <div style="flex:1;min-width:0"><div style="font-weight:700;font-size:12.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${d.numero||d.tipo_documento} ${d.firmado?'(firmado)':''}</div><div style="font-size:10.5px;color:var(--gris-400)">${d.tipo_documento} · ${new Date(d.created_at).toLocaleString('es-ES',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'})}</div></div>
  ...
  const presupHtml = (presups.data||[]).length ?
    ...
    (presups.data||[]).map(p=>`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--gris-100);cursor:pointer" onclick="verDetallePresupuesto(${p.id})">
  ...
  const albHtml = (albs.data||[]).length ?
    ...
    (albs.data||[]).map(a=>`
      <div style="display:flex;align-items:center;justify-content:space-between;padding:7px 0;border-bottom:1px solid var(--gris-100);cursor:pointer" onclick="verDetalleAlbaran(${a.id})">
    ...
    html += `<div style="padding:8px 10px;background:var(--gris-50);border-radius:7px;border:1.5px solid ${borderColor};margin-bottom:6px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        ...
        <div style="display:flex;gap:2px">
          ${!esPred?`<button onclick="_setPredeterminadaCli(${cb.id})" style="font-size:10px;color:var(--azul);background:#e8f0fe;border:1px solid var(--azul);border-radius:4px;cursor:pointer;padding:2px 6px;font-weight:600" title="Marcar como predeterminada">⭐ Predet.</button>`:''}
...
async function saveCliente() {
  ...
  const obj = {
    ...
    telefono: tel || null, movil: v('c_movil'), email: v('c_email'),
    direccion_fiscal: dir || null, municipio_fiscal: muni,
    cp_fiscal: cp || null, provincia_fiscal: prov,
...
async function delCliente(id) {
  // Comprobar vinculados
  const [{count:cT},{count:cP},{count:cA},{count:cF}] = await Promise.all([
    sb.from('trabajos').select('id',{count:'exact',head:true}).eq('cliente_id',id).neq('estado','eliminado'),
    sb.from('presupuestos').select('id',{count:'exact',head:true}).eq('cliente_id',id).neq('estado','eliminado'),
    sb.from('albaranes').select('id',{count:'exact',head:true}).eq('cliente_id',id).neq('estado','eliminado'),
    sb.from('facturas').select('id',{count:'exact',head:true}).eq('cliente_id',id).neq('estado','eliminado')
  ]);
  const total = (cT||0)+(cP||0)+(cA||0)+(cF||0);
  if (total > 0) {
