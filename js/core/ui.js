...
// (también incluimos modales con sus propios selects que no dependen de las
// listas globales — populateSelects es costoso si tienes muchos clientes/familias)
const _readOnlyModals = new Set(['dtlPartes','modal-detalle-trasp','modal-movimientos-stock','mArtVistaRapida','mImportarIA','mOcrPreview','mAutomatizacion','mReglaCorreo','mRetroactiva','mPrevisTarea']);
...
function goPage(id, opts){
  ...
  const titles={dashboard:'🏠 Panel de control',clientes:'👥 Clientes',proveedores:'🏭 Proveedores',articulos:'📦 Artículos',servicios:'🛠️ Servicios',almacenes:'🏬 Almacenes',asitur:'🛡️ Asitur',trabajos:'🏗️ Obras',mantenimientos:'🔧 Mantenimientos',presupuestos:'📋 Presupuestos',albaranes:'📄 Albaranes',facturas:'💶 Facturas','presupuestos-compra':'📋 Presupuestos de compra','pedidos-compra':'📦 Pedidos de compra','albaranes-proveedor':'📄 Albaranes de proveedor','facturas-proveedor':'🧾 Facturas de proveedor','calendario-pagos':'📅 Calendario de Pagos',correo:'📧 Correo',stock:'📊 Stock',traspasos:'🔄 Traspasos',activos:'🔧 Activos',partes:'📝 Partes de trabajo',formularios:'📋 Formularios',planificador:'⏱️ Planificador Semanal',fichajes:'⏱️ Fichajes','audit-log':'📜 Registro de actividad',papelera:'🗑️ Papelera',usuarios:'👷 Usuarios',configuracion:'⚙️ Configuración','etiquetas-qr':'🏷️ Etiquetas QR',mistareas:'✅ Tareas',calendario:'📅 Calendario',ocr:'🤖 Bandeja OCR',laboratorio:'🧪 Laboratorio de pruebas',flota:'🚐 Flota','flota-gastos':'💰 Gastos de flota','flota-gps':'📡 GPS en vivo','tesoreria-cuentas':'🏦 Cuentas bancarias','tesoreria-movimientos':'🏦 Movimientos','tesoreria-conciliacion':'🏦 Conciliación','tesoreria-importar':'🏦 Importar extractos','plan-contable':'📊 Plan Contable','libro-diario':'📖 Libro Diario','libro-mayor':'📒 Libro Mayor','balance-sumas':'📊 Balance de Sumas y Saldos','cuenta-resultados':'📈 Cuenta de Resultados',ausencias:'📋 Ausencias',timeline:'👥 Timeline operarios','calendario-laboral':'📅 Calendario laboral',mensajes:'💬 Mensajes'};
  ...
  if(id==='flota-gps' && typeof renderFlotaGPS==='function') renderFlotaGPS();
  ...
  if(id==='plan-contable' && typeof renderContPlanContable==='function') renderContPlanContable();
  if(id==='libro-diario' && typeof renderContLibroDiario==='function') renderContLibroDiario();
  if(id==='libro-mayor' && typeof renderContLibroMayor==='function') renderContLibroMayor();
  if(id==='balance-sumas' && typeof renderContBalance==='function') renderContBalance();
  if(id==='cuenta-resultados' && typeof renderContResultados==='function') renderContResultados();
  ...
  if(id==='mensajes' && typeof loadMensajesDesktop==='function') loadMensajesDesktop();
  if(id==='trabajos'){
